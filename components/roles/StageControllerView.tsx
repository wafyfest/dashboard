...
export type StageControllerTab = 'relay' | 'blind_allotment' | 'all_schedules';

interface StageControllerViewProps {
  activeTab?: StageControllerTab;
  onTabChange?: (tab: StageControllerTab) => void;
}

export function StageControllerView({ activeTab: controlledTab, onTabChange }: StageControllerViewProps = {}) {
  ...
  const [internalTab, setInternalTab] = useState<StageControllerTab>('relay');

  const activeTab = controlledTab ?? internalTab;
  const setActiveTab = (tab: StageControllerTab) => {
    if (onTabChange) {
      onTabChange(tab);
    } else {
      setInternalTab(tab);
  ...
  return (
    <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-300 text-[10px] font-mono">
                {activeTab === 'relay' ? 'LIVE RELAY' : activeTab === 'blind_allotment' ? 'BLIND ALLOTMENT' : 'AUDITORIUM SCHEDULES'}
              </span>
            </div>
            <h2 className="text-xl font-black mt-1">Stage Controller & Blind Allotment</h2>
            <p className="text-xs text-slate-300 mt-0.5">
              Active Stage: <strong>{currentStage?.name}</strong> ({currentStage?.location})
            </p>
          </div>
        </div>
      ...
      {/* 1. RELAY VIEW: Backstage Tablet & Queue */}
      {activeTab === 'relay' && (
      ...
      {/* 2. BLIND ALLOTMENT DEDICATED VIEW */}
      {activeTab === 'blind_allotment' && (
        <div className="space-y-6">
          <Card>
            <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  <CardTitle>Blind Judging Code Allotment Console</CardTitle>
                  <span className="px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-xs font-bold">
                    Active Item: {activeSchedule?.item?.name} ({activeSchedule?.item?.code})
                  </span>
                </div>
                <CardDescription>
                  Generate or manually adjust anonymous single-letter codes (A, B, C, D...) before stage call.
                </CardDescription>
              </div>
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="px-3 py-2 text-xs font-semibold bg-slate-50 border border-slate-200 rounded-xl text-[#132238] focus:ring-2 focus:ring-[#132238]/20"
                >
                  {stageSchedules.map(sch => (
                    <option key={sch.id} value={sch.id}>
                      {sch.item?.code} - {sch.item?.name}
                    </option>
                  ))}
                </select>
                <Button size="sm" onClick={handleAutoAllotCodes}>
                  <Shuffle className="w-3.5 h-3.5 mr-1" /> Randomize & Allot
                </Button>
              </div>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-24">Blind Code</TableHead>
                    <TableHead>Institution</TableHead>
                    <TableHead>Participant(s) & Chest Numbers</TableHead>
                    <TableHead className="text-right">Code Letter Assignment</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {activeRegistrations.length === 0 ? (
                    ...
                    activeRegistrations.map(reg => (
                      <TableRow key={reg.id}>
                        <TableCell>
                          {reg.code_letter ? (
                            <div className="w-10 h-10 rounded-xl bg-[#132238] text-white flex items-center justify-center font-black text-lg shadow-sm">
                              {reg.code_letter}
                            </div>
                          ) : (
                            <div className="w-10 h-10 rounded-xl bg-amber-100 text-amber-800 border border-amber-300 flex items-center justify-center font-bold text-xs">
                              Unset
                            </div>
                          )}
                        </TableCell>
                        <TableCell>
                          <div className="font-bold text-[#132238]">{reg.college?.code}</div>
                          <span className="text-xs text-slate-500">{reg.college?.name}</span>
                        </TableCell>
                        <TableCell>
                          <div className="text-xs font-semibold text-slate-800">
                            {reg.participants?.map(p => `${p.full_name} (${p.chest_no})`).join(', ') || 'No names'}
      ...
      {/* 3. ALL STAGE SCHEDULES TIMELINE VIEW */}
      {activeTab === 'all_schedules' && (
        <div className="space-y-6">
          <Card>
            <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <CardTitle>Festival Auditorium & Stage Schedules</CardTitle>
                <CardDescription>
                  Complete timeline across all venues and live event statuses
                </CardDescription>
              </div>
              <Badge variant="navy">
                <Calendar className="w-3.5 h-3.5 mr-1" />
                {allSchedules.length} Total Scheduled Events
              </Badge>
            </CardHeader>
            <CardContent className="p-0">
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>Live Stage Status</TableHead>
                    <TableHead className="text-right">Quick Control</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {allSchedules.map(sch => {
                    const stage = stages.find(s => s.id === sch.stage_id);
                    const isCurrentStage = sch.stage_id === selectedStageId;

                    return (
                      <TableRow key={sch.id} className={isCurrentStage ? 'bg-slate-50/50' : ''}>
                        <TableCell>
                          <div className="font-bold text-[#132238]">{stage?.name}</div>
                          <span className="text-[11px] text-slate-500">{stage?.location}</span>
                        </TableCell>
                        <TableCell>
                          <div className="font-mono text-xs font-bold text-slate-500">{sch.item?.code}</div>
                          <div className="font-bold text-sm text-[#132238]">{sch.item?.name}</div>
                        </TableCell>
                        <TableCell>
                          <Badge variant="outline" className="text-xs">
                            {sch.item?.category}
                          </Badge>
                          <span className="text-xs text-slate-500 ml-1.5">{sch.item?.item_type}</span>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-1 font-mono text-xs text-slate-700">
                            <Clock className="w-3.5 h-3.5 text-slate-400" />
                            {new Date(sch.scheduled_start || sch.starting).toLocaleTimeString([], {
                              hour: '2-digit',
                              minute: '2-digit'
                            })}
                          </div>
                        ...
                        <TableCell className="text-right">
                          <Button
                            ...
                            variant="outline"
                            onClick={() => {
                              if (sch.stage_id) setSelectedStageId(sch.stage_id);
